...
$sql = "select * from ques_data where c_no=4 order by rand() limit 5";
...
<html>
    <head>
        <title>Python Preperation Kit</title>
        <style>
form {
    margin-left:50pt;
    font-size:15pt;
...
#hm {
    ...
    <body>
        ...
        <br><br>
        <button type="submit" id="sub" onclick="<?php $i=0;$k=0;for ($i=0;$i<5;$i++){$k = $i + 1;$_SESSION['r' . $k] = $row[$i];$_SESSION['a' . $k] = $_POST['a' . $k];}$_SESSION['c_id'] = 4; ?>">Submit</button>
        </form>
    </body>
</html>